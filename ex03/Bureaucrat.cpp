...
/*   Updated: 2022/02/09 11:21:00 by adidion          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Bureaucrat::executeForm(Form const &a)
{
	try
	{
		if (a.getGradeToExecute() < this->_grade)
			throw (Bureaucrat::GradeTooLowException());
		if (this->_grade < 1)
			throw (Bureaucrat::GradeTooHighException());
		if (a.getIsSigned() == 0)
			std::cout << this->_name << " couldn't execute " << a.getName() << " beacause it isn't signed" << std::endl;
		a.execute(*this);
		return ;
	}
	catch (Bureaucrat::GradeTooLowException)
	{
		std::cout << this->_name << " couldn't execute " << a.getName() << " beacause grade is too low" << std::endl;
		return ;
	}
	catch (Bureaucrat::GradeTooHighException)
	{
		std::cout << this->_name << " couldn't execute " << a.getName() << " beacause grade is too high" << std::endl;
		return ;
	}
	std::cout << this->_name << " execute " << a.getName() << std::endl;;
